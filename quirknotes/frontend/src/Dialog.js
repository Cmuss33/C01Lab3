...
function Dialog({open, initialNote, closeDialog, postNote: postNoteState, patchNote: patchNoteState}) {
    ...
    const patchNote = (entry) => { // entry being passed is always null, so I used similar approach to postNote
        // Code for PATCH here
        if (!note || !note.title || !note.content) {
            return 
        }
        ...
        try {
            ...
            .then(async (response) => {
                if (!response.ok) {
                    ...
                    await response.json().then((data) => {
                        patchNoteState(note._id, note.title, note.content) // insertedID was undefined, so I used note._id instead
                        //setStatus("Note posted!") // Can be replaced with close(), if you want!
                        close()
                    }) 
                }
